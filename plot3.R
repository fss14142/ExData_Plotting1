...
if(!file.exists("plot3.R")){
    message("Are you executing this plot3.R script from your Working Directory?")
}
...
## For this script we will use columns 1,2 and 7 to 9.
columnsToOmit <- 3:6
...
## First we plot the Sub_metering_1 data: 
plot(datetime, hshldPwrConsumption$Sub_metering_1, 
     type="l",main="", xlab="", ylab="Energy sub metering")
## and now we add the remaining Sub_metering_2 and Sub_metering_3:
...
## and the selection of the variable names for the legend: 
legend("topright", legend=names(hshldPwrConsumption)[-(1:2)], lty=c(1,1,1), 
       col=c("black","red","blue"), cex=.97)
